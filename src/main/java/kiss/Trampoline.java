 ...
 * @see <a href="https://mrbackend.github.io/java-trampoline/index.html">java-trampoline web
 *      site</a>
 */
public abstract class Trampoline<A> {
    ...
    /**
     * Creates a {@code Trampoline} that represents a thunk (an unevaluated calculation that takes
     * no parameters).
     * <p>
     * When the returned trampoline is run, it will evaluate {@code thunk} to get a new trampoline
     * and then run that one to get and return its result.
     * <p>
     * The purpose of {@code suspend} is to avoid an immediate recursion, instead returning a
     * trampoline that represents the recursion to be run later.
     *
    ...
    /**
     * Evaluates this {@code Trampoline} in constant stack space (disregarding the stack space that
     * is consumed by the functions used to construct this {@code Trampoline}).
     * <p>
     * This is achieved by evaluating each of the functions and sub-trampolines used to create the
     * trampoline. The point is that each of these steps is done in a loop, such that the stack does
     * not grow during the evaluation.
     *
    ...
    /**
     * Creates a {@code Trampoline} that represents the application of {@code transformValue} to the
     * result of this trampoline.
     * <p>
     * When the returned trampoline is run, it will first run this trampoline, then call
     * {@code transformValue} and return its result.
     * <p>
     * The purpose of {@code map} is to transform the resulting value of a trampoline. The transform
     * must be stack safe; if a deep recursion is performed to transform the value, it should be
     * wrapped in a trampoline instead, and passed to {@code flatMap} instead of {@code map}.
     *
     * @param transformValue The function used to transform the result of this Trampoline
     * @param <B> The transformed value's type
     * @return A new Trampoline that represents the application of transformValue to the result of
     *         this Trampoline
    ...
    /**
     * Creates a {@code Trampoline} that represents the application of {@code calcNextTrampoline} to
     * the result of this trampoline.
     * <p>
     * When the returned trampoline is run, it will first run this trampoline, then call
     * {@code getNextTrampoline} to get a new trampoline, and finally run the new trampoline and
     * return its result.
     * <p>
     * The purpose of {@code flatMap} is to chain two recursions where the second recursion depends
     * on the first one.
     *
     * @param calcNextTrampoline The function used to calculate the next Trampoline
     * @param <B> The type of the value returned by the next Trampoline (and, consequently, the
     *            returned Trampoline)
     * @return A new Trampoline that represents the sequence of this Trampoline followed by the next
     *         Trampoline
     ...
     * FlatMap(trampoline, value -> FlatMap(calcNextTrampoline.apply(value),
     * parentCalcNextTrampoline)) Throws IllegalStateException if called for a Return(value)
     */
