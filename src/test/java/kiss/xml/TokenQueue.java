...
public class TokenQueue {
    ...
    public String consumeToIgnoreCase(String seq) {
        ...
        boolean canScan = first.toLowerCase().equals(first.toUpperCase()); // if first is not cased,
                                                                           // use index of
        while (!isEmpty()) {
     ...
     * be escaped (with \). Those escapes will be left in the returned string, which is suitable for
     * regexes (where we need to preserve the escape), but unsuitable for contains text strings; use
     * unescape for that.
     * 
     * @param open opener
     * @param close closer
    ...
    public String chompBalanced(char open, char close) {
        ...
        do {
            ...
            if (depth > 0 && last != 0) accum.append(c); // don't include the outer match pair in
                                                         // the return
            last = c;
        } while (depth > 0);
    ...
    /**
     * Consume a CSS element selector (tag name, but | instead of : for namespaces, to not conflict
     * with :pseudo selects).
     * 
     * @return tag name
     */
